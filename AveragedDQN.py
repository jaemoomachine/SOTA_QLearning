...
class AveragedDQNAgent:
    def __init__(self, state_dim, action_dim, K=K):
        ...
        self.avg_models = deque(maxlen=K)
    ...
    def train(self):
        ...
        with torch.no_grad():
            if self.avg_models:
                q_targets = []
                for avg_model in self.avg_models:
                    q_next = avg_model(s_)
                    q_next_max = q_next.max(dim=1)[0]
                    q_targets.append(q_next_max)
                avg_q_next = torch.stack(q_targets).mean(dim=0)
            else:
                avg_q_next = self.model(s_).max(dim=1)[0]

            q_target = r + self.gamma * avg_q_next * (~d)
    ...
    def update_avg_models(self):
        ...
        copied_model.eval()
        self.avg_models.append(copied_model)
...
def train_and_test(df, max_episodes=100):
    ...
    agent = AveragedDQNAgent(train_f.shape[1], len(actions), K=K)
