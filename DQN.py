...
class DQNAgent:
    ...
    def train(self):
        ...
        q_next = self.target_model(s_).max(1)[0].detach()
        q_target = r + self.gamma * q_next * (~d)
...
def train_and_test(df, max_episodes=100):
    ...
    save_model(agent, path=f"{global_path}/Models/DQN.pth")
    ...
    pd.DataFrame({"Test Action Weight": [actions[a] for a in test_actions], "Test Reward": test_rewards}).to_csv(f"{global_path}/Results/DQN_test_actions_rewards.csv", index=False)
    pd.DataFrame({"Train Episode Reward": train_rewards}).to_csv(f"{global_path}/Results/DQN_train_episode_rewards.csv", index=False)
    pd.DataFrame({"Action Index": list(range(len(actions))), "Action Weight": actions, "Action Count": action_counts}).to_csv(f"{global_path}/Results/DQN_action_counts.csv", index=False)
...
if __name__ == "__main__":
    ...
    train_and_test(df, max_episodes=1000)