...
# -------------------- Adaptive Order Q-learning Agent --------------------
class AdaptiveOrderDQNAgent:
    def __init__(self, state_dim, action_dim, K=K, beta=0.999):
        self.model = DQN(state_dim, action_dim).to(device)
        self.model_pool = deque(maxlen=K)
    ...
    def get_adaptive_k(self):
        k = int(np.ceil(self.K * (1 - self.beta ** self.training_step)))
        current_pool_size = len(self.model_pool)
        return max(1, min(current_pool_size, k))
        ...
        with torch.no_grad():
            if self.model_pool:
                ...
                for model in self.model_pool:
                    ...
                    q_targets.append(q_next_max)
                sorted_q = torch.stack(q_targets).sort(dim=0)[0]
                k = self.get_adaptive_k()
                selected_q = sorted_q[k - 1]
            else:
                selected_q = self.model(s_).max(dim=1)[0]

            q_target = r + self.gamma * selected_q * (~d)
    ...
    def update_model_pool(self):
        copied_model = DQN(self.model.net[0].in_features, self.model.net[-1].out_features).to(device)
        copied_model.load_state_dict(self.model.state_dict())
        copied_model.eval()
        self.model_pool.append(copied_model)
...
def train_and_test(df, max_episodes=100):
    ...
    agent = AdaptiveOrderDQNAgent(train_f.shape[1], len(actions), K=K, beta=0.999)
